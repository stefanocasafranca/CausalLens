...
\begin{itemize}
    \item \textbf{Matrix Factorization (MF)}~\cite{koren2009matrix}: 64-dimensional user/item factors trained with SGD for 20 epochs (batch size 1024). MF's linear structure provides a simpler baseline: adversary perturbations trigger 25-step warm-start retrains of shared factors, but the linear model limits how far cross-user effects propagate compared to deeper architectures.
